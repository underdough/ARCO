# 📚 Índice de Documentación - Sistema ARCO

Navegación rápida por toda la documentación del sistema ARCO.

## 🚀 Inicio Rápido

- **[Inicio Rápido](INICIO_RAPIDO.md)** - Configuración en 5 minutos ⚡
- **[Instrucciones de Instalación MVP](INSTRUCCIONES_INSTALACION_MVP.md)** - Instalación paso a paso

## 📧 Configuración de Email

- **[Cómo Configurar Email](COMO_CONFIGURAR_EMAIL.md)** - Guía visual paso a paso
- **[Configuración Email Producción](configuracion_email_produccion.md)** - Guía completa SMTP para producción

## 🏗️ Arquitectura y Diseño

- **[Arquitectura del Sistema](arquitectura_sistema.md)** - Diseño técnico y componentes
- **[Especificación de Requerimientos (SRS)](especificacion_requerimientos_software.md)** - IEEE 830 - Requerimientos completos

## 🔐 Seguridad y Autenticación

- **[Sistema 2FA](SISTEMA_2FA.md)** - Autenticación de dos factores
- **[Sistema de Permisos](SISTEMA_PERMISOS.md)** - Gestión de roles y acceso

## 📖 Guías de Uso

- **[Guía General del Sistema](SISTEMA_ARCO_GUIA.md)** - Guía completa de uso
- **[Solución de Problemas](SOLUCION_PROBLEMAS.md)** - Troubleshooting y soluciones

## 📋 Información Adicional

- **[Requerimientos Faltantes](proyecto_requerimientos_faltantes.txt)** - Listado de requerimientos pendientes

## 🧪 Pruebas

Ver carpeta `/tests` para archivos de prueba:
- [Tests README](../tests/README.md) - Documentación de pruebas

## 📁 Estructura de Documentación

```
/documentacion/
├── INDICE_DOCUMENTACION.md          # Este archivo
├── INICIO_RAPIDO.md                 # Configuración rápida
├── INSTRUCCIONES_INSTALACION_MVP.md # Instalación completa
├── COMO_CONFIGURAR_EMAIL.md         # Guía de email
├── configuracion_email_produccion.md # Email para producción
├── arquitectura_sistema.md          # Diseño técnico
├── especificacion_requerimientos_software.md # SRS IEEE 830
├── SISTEMA_2FA.md                   # Autenticación 2FA
├── SISTEMA_PERMISOS.md              # Gestión de permisos
├── SISTEMA_ARCO_GUIA.md             # Guía general
├── SOLUCION_PROBLEMAS.md            # Troubleshooting
├── instalar_estadisticas.sql        # Script de BD
└── proyecto_requerimientos_faltantes.txt # Requerimientos
```

## 🎯 Guías por Rol

### Administrador del Sistema
1. [Inicio Rápido](INICIO_RAPIDO.md)
2. [Arquitectura del Sistema](arquitectura_sistema.md)
3. [Sistema de Permisos](SISTEMA_PERMISOS.md)
4. [Configuración Email Producción](configuracion_email_produccion.md)

### Gerente de Almacén
1. [Guía General del Sistema](SISTEMA_ARCO_GUIA.md)
2. [Inicio Rápido](INICIO_RAPIDO.md)
3. [Solución de Problemas](SOLUCION_PROBLEMAS.md)

### Supervisor
1. [Guía General del Sistema](SISTEMA_ARCO_GUIA.md)
2. [Sistema de Permisos](SISTEMA_PERMISOS.md)

### Almacenista
1. [Guía General del Sistema](SISTEMA_ARCO_GUIA.md)

## 🔍 Búsqueda por Tema

### Instalación y Configuración
- [Inicio Rápido](INICIO_RAPIDO.md)
- [Instrucciones de Instalación MVP](INSTRUCCIONES_INSTALACION_MVP.md)
- [Cómo Configurar Email](COMO_CONFIGURAR_EMAIL.md)
- [Configuración Email Producción](configuracion_email_produccion.md)

### Seguridad
- [Sistema 2FA](SISTEMA_2FA.md)
- [Sistema de Permisos](SISTEMA_PERMISOS.md)
- [Especificación de Requerimientos](especificacion_requerimientos_software.md)

### Operación
- [Guía General del Sistema](SISTEMA_ARCO_GUIA.md)
- [Solución de Problemas](SOLUCION_PROBLEMAS.md)

### Desarrollo
- [Arquitectura del Sistema](arquitectura_sistema.md)
- [Especificación de Requerimientos](especificacion_requerimientos_software.md)

## 📞 Soporte

Para problemas específicos, consultar:
1. [Solución de Problemas](SOLUCION_PROBLEMAS.md) - Problemas comunes
2. [Guía General del Sistema](SISTEMA_ARCO_GUIA.md) - Funcionalidades
3. [Configuración Email Producción](configuracion_email_produccion.md) - Problemas de email

## 📝 Notas

- Toda la documentación está en español
- Se recomienda leer [Inicio Rápido](INICIO_RAPIDO.md) primero
- Para producción, seguir [Configuración Email Producción](configuracion_email_produccion.md)
- Consultar [Solución de Problemas](SOLUCION_PROBLEMAS.md) ante cualquier inconveniente

---

**Última actualización:** Diciembre 2025
**Versión del Sistema:** 2.0.0
